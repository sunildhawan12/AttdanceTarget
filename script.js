...
async function submitAttendance(status) {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    if (studentMap[id]) {
      msg.innerHTML = `✅ Hello! <b style="color:#2193b0">${studentMap[id].name}</b> ${status} marked at 🕒 ${timeNow}`;
      loading.style.display = "none";
...
async function filterByDate() {
  ...
  try {
    ...
    // ✅ Sort by real date object - latest first
    allData.sort((a, b) => {
    ...
    // ✅ Highlight the latest date row
    const latestDate = filtered.length > 0 ? filtered[0].date : null;
    const firstRow = allData[0]; // latest date after sorting
    latestDateElement.innerHTML = `🗓️ Latest Attendance Date: <span style="color:#2193b0">${firstRow.date}</span>`;
